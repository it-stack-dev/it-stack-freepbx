# Troubleshooting — IT-Stack FREEPBX
...
1. Check logs: docker compose logs freepbx
2. Verify environment variables are set correctly
3. Check database connectivity: pg_isready -h lab-db1 -p 5432
4. Verify port is not already in use: ss -tlnp | grep 5060
...
```bash
# Test connectivity
psql -h lab-db1 -U freepbx_user -d freepbx_db -c '\conninfo'

# Check pg_hba.conf allows connection from lab-pbx1
```

### Performance issues

1. Check resource usage: docker stats it-stack-freepbx
...
| Log | Path |
|-----|------|
| Application | docker compose logs freepbx |
